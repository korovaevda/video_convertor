...
# Вычисляем начало fade out используя Python (доступен везде)
fade_start=$(python3 -c "print($video_duration - $FADE_DURATION)")
echo "Fade out начнётся с: ${fade_start%.*} сек"

# Проверяем, нужно ли зациклить аудио (используем Python для сравнения)
audio_shorter=$(python3 -c "print(1 if $audio_duration < $video_duration else 0)")

if [ "$audio_shorter" = "1" ]; then
